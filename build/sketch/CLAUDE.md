#line 1 "/home/cursor/SECURITY/ble-scanner/CLAUDE.md"
# CLAUDE.md
...
### The Solution: BLE Deinit/Reinit Cycle

**The only working solution** is to completely deinitialize the BLE stack before each HTTPS POST, then reinitialize it afterward:

```cpp
void postLogsToServer() {
  // ... validation checks ...

  // CRITICAL: Completely deinit BLE to free memory for HTTPS
  Serial.println("Deiniting BLE for HTTP POST...");
  pBLEScan->stop();
  BLEDevice::deinit(true);  // true = release all BLE memory
  delay(100);

  // Now heap has ~162KB free with ~45KB contiguous - enough for SSL!

  // ... build JSON payload ...
  // ... do HTTPS POST ...

  // Reinitialize BLE after POST
...
### Memory Usage During POST

With the deinit/reinit solution:

```
Server URL: https://ble-scanner.fly.dev/api/logs
Deiniting BLE for HTTP POST...
Free heap after BLE deinit: 162920 bytes, Largest block: 45044 bytes
Posting 10 devices to server (payload: 984 bytes)...
Server post OK: 10 devices sent
Reinitializing BLE...
BLE reinitialized.
```

### Additional Optimizations Used

1. **StaticJsonDocument** - Use stack allocation instead of heap for JSON:
   ```cpp
   StaticJsonDocument<2048> doc;  // Stack, not heap
   char payload[2048];            // Stack buffer for serialization
   ```

2. **Limit devices per POST** - Send max 10 devices to keep payload small:
   ```cpp
   int devicesToSend = min(deviceCount, 10);
   ```

3. **Stack-based payload buffer** - Avoid String class heap allocation:
   ```cpp
   char payload[2048];
...
- The server runs plain HTTP on port 8080
- Fly.io terminates SSL at the edge and forwards HTTP to the app
- Fly.io REQUIRES HTTPS (redirects HTTP 301 → HTTPS)
- The API key is sent via `Authorization: Bearer <key>` header
- Server endpoint: `DELETE /api/logs` clears all logs (for testing)

### Alternative Approaches (Not Implemented)

If the deinit/reinit approach is too slow or causes issues:

1. **MQTT instead of HTTPS** - Lower memory overhead, persistent connection
2. **Local HTTP proxy** - Raspberry Pi accepts HTTP, forwards to Fly.io via HTTPS
3. **Different hosting** - Platform that allows plain HTTP (not Fly.io)
4. **Batch less frequently** - POST every 5 minutes instead of every scan cycle
...
4. **`BLEDevice::deinit(true)` is the nuclear option** - But it works!
5. **Test HTTPS before and after BLE init** - Isolate the problem
